...
Rcpp::List plausMC(NumericVector theta, NumericVector intcpt, NumericMatrix grid, NumericVector stat, NumericVector del, NumericVector df, int m_samps, bool intercept){
	...
	if(intercept){
		...
		NumericVector Z(m_samps, 0.0); Z = Rcpp::rnorm(m_samps,0.0,1.0);
				...
				for(int j = 0; j < m_grid; j++){
					...
					if(L22[0] > 0.0){
						...
						for(int k = 0; k< m_samps; k++){
							...
							dens3[0] = dens1[0]*R::dnorm(Z[k], 0.0, 1.0, 0);
							dens4[0] = dens2[0]*R::dnorm((ybar[0] - intcpt[l]-theta[i]*wbar[0])/std::sqrt(std::pow(L12[0]*theta[i]+L11[0], 2)+std::pow(theta[i]*L22[0], 2)), 0.0, 1.0, 0);
							if( dens1[0] <= dens2[0] ){
		...
		result = Rcpp::List::create(Rcpp::Named("plauses.theta") = plaus_theta, Rcpp::Named("plauses.intercept") = plaus_intcpt);
	}else {
		for(int i = 0; i < m_the; i++){
			for(int j = 0; j < m_grid; j++){
				L11[0] = std::sqrt( grid(j, 1) + grid(j, 0)*theta[i]*theta[i] );
