	...
	if(marginalize){
		if(intercept){
			...
			for(int i = 0; i < m_the; i++){
				for(int k = 0; k< m_samps; k++){
					if(theta[i] <= aux_var[k]){
						F_theta[i] = F_theta[i] + 1.0/m_the; 	
					}
				}
				plaus_theta[i] = 1.0 - std::abs(2.0*F_theta[i] - 1.0);
			}
			NumericVector F_intcpt(m_the,0.0);
			for(int i = 0; i < m_int; i++){
				for(int k = 0; k< m_samps; k++){
					if(intcpt[i] <= aux_var2[k]){
						F_intcpt[i] = F_intcpt[i] + 1.0/m_int; 	
					}
				}
				plaus_intcpt[i] = 1.0 - std::abs(2.0*F_intcpt[i] - 1.0);
			}			
