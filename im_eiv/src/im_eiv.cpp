...
Rcpp::List plauscontourSIR(NumericVector sampsize, NumericVector stat, NumericVector del, NumericVector n, NumericVector mode, NumericVector dens, NumericVector se2, NumericVector cond_par) {
	...
	for(int k = 0; k < ct; k++){
		densx[k] = log(std::abs(1.0/cond_par[2]))+R::dnorm((cond_par[0] -cond_par[1]*std::sqrt(tV1[k]) - std::sqrt(tV3[k]))/cond_par[2], 0.0, 1.0, 1) + R::dchisq(tV1[k], n[0]-2.0, 1) + R::dchisq(tV3[k], n[0]-3.0, 1);	
	...
	for(int k = 0; k < size; k++){
		weights[k] = std::exp(log(std::abs(1.0/cond_par[2]))+R::dnorm((cond_par[0] -cond_par[1]*V1[k] - V3[k])/cond_par[2], 0.0, 1.0, 1) + R::dchisq(V1[k]*V1[k], n[0]-2.0, 1) + R::dchisq(V3[k]*V3[k], n[0]-3.0, 1) - R::dchisq(V1[k]*V1[k], mode[0], 1) - R::dchisq(V3[k]*V3[k], mode[1], 1));
	...
	for(int k = 0; k < size; k++){
		...
		samples(k,5) = log(std::abs(1.0/cond_par[2]))+R::dnorm((cond_par[0] -cond_par[1]*sV1[k] - sV3[k])/cond_par[2], 0.0, 1.0, 1) + R::dchisq(sV1[k]*sV1[k], n[0]-2.0, 1) + R::dchisq(sV3[k]*sV3[k], n[0]-3.0, 1);	
